...
Post-completion vocabulary refinement (July 12, 2026): the shared Browse/Studio taxonomy is Music—Album, EP, Single, Mixtape, Live Set; Books—Novel, Artbook, Zine; Apparel—Apparel, Accessories, Physical Music, Goods & Collectibles; Assets—Sample Packs, Remix Packs, Game Assets. Front-facing Store/Home labels are now Browse while `/store` remains the stable internal route and domain name. Filtered catalog headings say Browse, and capability is presented as Features with Achievements as the only launch option.
