...
- [ ] Publish the reviewed `.github/workflows/desktop-build.yml` after the repository credential receives GitHub's separate `workflow` scope; the accepted installer artifact does not depend on that source file at runtime.
- [ ] Add a macOS job only if it provides useful reproducibility; the first Mac artifact may be built locally.
- [x] The reviewed workflow source specifies `windows-latest` x64 with Node LTS, Rust stable, npm cache, `npm ci`, desktop contracts, Rust formatting/Clippy/tests, and the Tauri build.
- [x] The reviewed workflow source is manual/private and does not run on ordinary `main` web pushes.
- [ ] Run the native Windows workflow after it is published; the current public installer was produced through the documented local cross-build fallback and still requires real-device acceptance.
...
- Completed: exact Tauri CLI and locked Rust scaffold, icons, origin-bound notification capability, desktop security contract, public Windows installer artifact, universal Mac `.app`/DMG build, production dependency patches, shared signed-out Account auth, catalog-query batching, and marketing/application asset isolation.
...
- Evidence: production npm audit reports zero vulnerabilities; web lint, strict typecheck, production build, focused contracts, Rust formatting, Clippy, and Rust tests pass. A local production-host comparison proves marketing omits the generated application stylesheet and `app-frame` while the app receives both. The ad-hoc-signed `44OS.app` is a valid arm64/x86_64 Mach-O bundle with identifier `com.fortyfour.os44`; it launched the live application successfully, and the integrated transparent title bar passed visual inspection. The cache-safe `44OS-0.1.0-mac-universal-r2.dmg` retains that accepted application bundle and adds the reviewed production-derived radial background, centered drag-to-Applications layout, and custom 44OS hard-drive volume icon; its SHA-256 is `840ba40509f16421567a414c917b7c10a4befaf80190b67d1896a7ec11c29644`. Finder may suppress folder backgrounds and saved icon placement when a system is configured to open mounted volumes inside an existing tab; the installer remains functional in that generic Finder presentation.
- Decisions: keep Tauri rather than Electron, exact `https://app.44os.com/`, only the origin-bound desktop notification bridge as native capability, no updater, ad-hoc Mac and unsigned Windows distribution, shared first-party analytics consent, and four `Other` handoff documents.
- Accepted: the public Mac and Windows downloads are live. The Mac DMG completes download, mount, drag-to-Applications, launch, and native-notification permission/display on the owner’s Apple Silicon Mac; the custom mounted-volume icon is visually accepted.
- Remaining acceptance: complete the broader Mac parity/uninstall matrix and the real-device Windows SmartScreen, install, launch, navigation, Auth, playback, upload/download, checkout, notification, and uninstall matrix. Any failure keeps the affected artifact available only after an explicit release decision or rolls its public link back while **Open App** remains available.
