...
- Evidence: production npm audit reports zero vulnerabilities; web lint, strict typecheck, production build, focused contracts, Rust formatting, Clippy, and Rust tests pass. A local production-host comparison proves marketing omits the generated application stylesheet and `app-frame` while the app receives both. The ad-hoc-signed `44OS.app` is a valid arm64/x86_64 Mach-O bundle with identifier `com.fortyfour.os44`; it launched the live application successfully, and the integrated transparent title bar passed visual inspection. The published universal DMG retains that accepted application bundle and now adds the reviewed production-derived radial background, centered drag-to-Applications layout, and custom 44OS hard-drive volume icon. Finder may suppress folder backgrounds and saved icon placement when a system is configured to open mounted volumes inside an existing tab; the installer remains functional in that generic Finder presentation.
