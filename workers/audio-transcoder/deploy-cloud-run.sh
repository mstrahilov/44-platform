...
GCP_REPOSITORY="${GCP_REPOSITORY:-fortyfour-workers}"
GCP_JOB_NAME="${GCP_JOB_NAME:-fortyfour-audio-transcoder}"
GCP_IMAGE="${GCP_REGION}-docker.pkg.dev/${GCP_PROJECT_ID}/${GCP_REPOSITORY}/${GCP_JOB_NAME}:latest"
WORKER_SERVICE_ACCOUNT_NAME="${WORKER_SERVICE_ACCOUNT_NAME:-fortyfour-audio-worker}"
SCHEDULER_SERVICE_ACCOUNT_NAME="${SCHEDULER_SERVICE_ACCOUNT_NAME:-fortyfour-audio-scheduler}"
WORKER_SERVICE_ACCOUNT="${WORKER_SERVICE_ACCOUNT_NAME}@${GCP_PROJECT_ID}.iam.gserviceaccount.com"
SCHEDULER_SERVICE_ACCOUNT="${SCHEDULER_SERVICE_ACCOUNT_NAME}@${GCP_PROJECT_ID}.iam.gserviceaccount.com"
...
gcloud iam service-accounts describe "${WORKER_SERVICE_ACCOUNT}" >/dev/null 2>&1 \
  || gcloud iam service-accounts create "${WORKER_SERVICE_ACCOUNT_NAME}" --display-name "44 audio transcoder"
gcloud iam service-accounts describe "${SCHEDULER_SERVICE_ACCOUNT}" >/dev/null 2>&1 \
  || gcloud iam service-accounts create "${SCHEDULER_SERVICE_ACCOUNT_NAME}" --display-name "44 audio scheduler"
...
for secret_name in 44-supabase-url 44-supabase-service-role; do
...
gcloud run jobs add-iam-policy-binding "${GCP_JOB_NAME}" --region "${GCP_REGION}" \
...
gcloud scheduler jobs describe "${SCHEDULER_NAME}" --location "${GCP_REGION}" >/dev/null 2>&1 \
  && gcloud scheduler jobs update http "${SCHEDULER_NAME}" --location "${GCP_REGION}" --schedule '* * * * *' --uri "${SCHEDULER_URI}" --http-method POST --oauth-service-account-email "${SCHEDULER_SERVICE_ACCOUNT}" --oauth-token-scope "https://www.googleapis.com/auth/cloud-platform" \
  || gcloud scheduler jobs create http "${SCHEDULER_NAME}" --location "${GCP_REGION}" --schedule '* * * * *' --uri "${SCHEDULER_URI}" --http-method POST --oauth-service-account-email "${SCHEDULER_SERVICE_ACCOUNT}" --oauth-token-scope "https://www.googleapis.com/auth/cloud-platform"
