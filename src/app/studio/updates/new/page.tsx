...
export default function NewUpdatePage() {
  ...
  if (loading) return <PageShell><div style={{ minHeight: '40vh' }} /></PageShell>;
  ...
  return <PageShell><div className="dashboard-page"><HubHero title="New Update" copy="Share an update about one of your releases." />
    <form className="dashboard-form" onSubmit={submit}>
      <section className="dashboard-form-section"><div className="dashboard-form-step">
        <label className="dashboard-field"><span className="dashboard-field-label">Release</span><select className="os-input-field" value={itemId} onChange={event => setItemId(event.target.value)}><option value="">Select a release</option>{items.map(item => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label>
