...
const blank = (): Draft => ({ title:'',shortDescription:'',format:'',startLocal:'',endLocal:'',timezone:browserTimeZone(),venueName:'',address1:'',address2:'',locality:'',region:'',postalCode:'',countryCode:'',onlineUrl:'',infoUrl:'' });
...
export default function StudioEventEditor() {
  ...
  async function submit(e:React.FormEvent){e.preventDefault();setSaving(true);setError('');try{if(!draft.format)throw new Error('Choose an event format.');const starts=instantFromLocalInput(draft.startLocal,draft.timezone);const ends=draft.endLocal?instantFromLocalInput(draft.endLocal,draft.timezone):null;if(ends&&new Date(ends)<=new Date(starts))throw new Error('End must be after start.');await saveCreatorEvent(isNew?null:id,{title:draft.title,short_description:draft.shortDescription,format:draft.format,starts_at:starts,ends_at:ends,timezone:draft.timezone,venue_name:draft.format==='online'?null:draft.venueName,address_line1:draft.format==='online'?null:draft.address1,address_line2:draft.format==='online'?null:draft.address2,locality:draft.format==='online'?null:draft.locality,region:draft.format==='online'?null:draft.region,postal_code:draft.format==='online'?null:draft.postalCode,country_code:draft.format==='online'?null:draft.countryCode.toUpperCase(),online_url:draft.format==='in_person'?null:draft.onlineUrl,info_url:draft.infoUrl});clearStudioFormRecovery(recoveryKey);router.push('/studio/events');}catch(saveError){setError(saveError instanceof Error?saveError.message:'Could not save event.');setSaving(false);}}
  if (!ready) return <PageShell><main className="dashboard-editor"><HubHero title={isNew ? 'New Event' : 'Edit Event'} /><div className="ui44-state ui44-state-loading" role="status" aria-live="polite">Loading...</div></main></PageShell>;
              ...
              <div className="dashboard-form-step ui44-panel ui44-panel-glass ui44-panel-overflow-visible">
                <div className="dashboard-form-grid dashboard-form-grid-2 ui44-form-grid">
                  <label className="dashboard-field">
                    ...
                    <Ui44TextInput className="os-input-field" required maxLength={120} value={draft.title} placeholder="Enter event title" onChange={event => set('title', event.target.value)} />
                  </label>
                  <label className="dashboard-field">
                    <div className="dashboard-field-label">Format</div>
                    <Ui44SelectInput value={draft.format} onChange={event => set('format', event.target.value)}>
                ...
                <label className="dashboard-field">
                  ...
                  <Ui44Textarea className="os-input-field dashboard-textarea" required maxLength={280} value={draft.shortDescription} placeholder="Enter event description" onChange={event => set('shortDescription', event.target.value)} />
                </label>
              </div>
            </section>
              ...
              <div className="dashboard-form-step ui44-panel ui44-panel-glass ui44-panel-overflow-visible">
                <div className="dashboard-form-grid dashboard-form-grid-2 ui44-form-grid">
                  <label className="dashboard-field">
                    ...
                    <Ui44TextInput type="datetime-local" className="os-input-field" required value={draft.startLocal} onChange={event => set('startLocal', event.target.value)} />
                  </label>
                  <label className="dashboard-field">
                    <div className="dashboard-field-label">End (Optional)</div>
                    <Ui44TextInput type="datetime-local" className="os-input-field" value={draft.endLocal} onChange={event => set('endLocal', event.target.value)} />
                  </label>
                </div>
              </div>
            </section>

            {(draft.format === 'in_person' || draft.format === 'hybrid') && <section className="dashboard-form-section">
              <SectionHeader title="Location" description="Add the venue and address people should use." />
              <div className="dashboard-form-step ui44-panel ui44-panel-glass ui44-panel-overflow-visible">
                <div className="dashboard-form-grid dashboard-form-grid-2 ui44-form-grid">
                  <label className="dashboard-field"><div className="dashboard-field-label">Venue</div><Ui44TextInput required className="os-input-field" value={draft.venueName} placeholder="Enter venue name" onChange={event => set('venueName', event.target.value)} /></label>
                  <label className="dashboard-field"><div className="dashboard-field-label">Address</div><Ui44TextInput required className="os-input-field" value={draft.address1} placeholder="Enter street address" onChange={event => set('address1', event.target.value)} /></label>
                  <label className="dashboard-field"><div className="dashboard-field-label">City</div><Ui44TextInput required className="os-input-field" value={draft.locality} placeholder="Enter city" onChange={event => set('locality', event.target.value)} /></label>
                </div>
              </div>
            </section>}

            {(draft.format === 'online' || draft.format === 'hybrid') && <section className="dashboard-form-section">
              ...
              <div className="dashboard-form-step ui44-panel ui44-panel-glass ui44-panel-overflow-visible">
                <label className="dashboard-field">
                  ...
                  <Ui44TextInput type="url" required className="os-input-field" value={draft.onlineUrl} placeholder="Enter online event URL" onChange={event => set('onlineUrl', event.target.value)} />
                </label>
              </div>
            </section>}
            ...
            <section className="dashboard-form-section">
              ...
              <div className="dashboard-form-step ui44-panel ui44-panel-glass ui44-panel-overflow-visible">
                <label className="dashboard-field">
                  ...
                  <Ui44TextInput type="url" className="os-input-field" value={draft.infoUrl} placeholder="Enter event information URL" onChange={event => set('infoUrl', event.target.value)} />
                </label>
