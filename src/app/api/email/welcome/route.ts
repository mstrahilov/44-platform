...
import { applicationOrigin, commerceAdminClient } from '@/lib/server/commerce';
...
export async function POST(request: Request) {
  try {
    ...
    const queued = await commerceAdminClient().rpc('queue_welcome_email' as never, { target_user_id: user.id, target_library_url: `${applicationOrigin()}/library` } as never);
