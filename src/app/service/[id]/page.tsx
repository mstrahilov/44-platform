      ...
      <div className="product-shell">
          ...
          <section style={{ minHeight: 460, borderRadius: 30, border: '1px solid rgba(255,255,255,0.10)', overflow: 'hidden', position: 'relative', background: 'linear-gradient(135deg, rgba(28,42,66,0.94), rgba(10,10,18,0.98)), radial-gradient(circle at 75% 20%, rgba(147,255,0,0.22), transparent 34%)' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1, minHeight: 460, padding: 36, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
              <div>
                ...
                <div style={{ fontSize: 18, fontWeight: 650, color: 'rgba(255,255,255,0.62)' }}>by {service.creators?.name ?? '44 Creator'}</div>
              </div>
        ...
        <aside style={{ position: 'sticky', top: 0, display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ borderRadius: 24, border: '1px solid rgba(255,255,255,0.10)', background: 'rgba(255,255,255,0.055)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', padding: 22 }}>
            <div style={{ fontSize: 30, fontWeight: 800, color: '#fff', letterSpacing: '-0.03em', marginBottom: 6 }}>{formatServicePrice(service)}</div>
            <div style={{ fontSize: 12, fontWeight: 600, color: 'rgba(255,255,255,0.34)', marginBottom: 16 }}>{service.delivery_estimate ?? 'Timeline varies'}</div>
            <textarea
              value={message}
              onChange={event => setMessage(event.target.value)}
              placeholder="Tell the creator what you need..."
              style={{ width: '100%', minHeight: 132, resize: 'vertical', background: 'rgba(0,0,0,0.25)', border: '1px solid rgba(255,255,255,0.10)', borderRadius: 16, padding: 14, fontFamily: 'inherit', fontSize: 13, fontWeight: 500, color: '#fff', outline: 'none', lineHeight: 1.5, marginBottom: 12 }}
            />
            <button className="btn-primary" onClick={submitRequest} style={{ width: '100%' }}>{requested ? 'Request Sent' : 'Send Request'}</button>
          </div>

          <InfoPanel title="Creator">
            <InfoLine label="Name" value={service.creators?.name ?? '44 Creator'} />
