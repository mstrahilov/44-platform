...
function CreatorDirectory() {
  ...
  useEffect(() => {
    ...
    void listTeamCreators({ query: submittedQuery, sort }).then(data => {
  ...
  return <PageShell><main className="team-page">
    <HubHero title="Creators" copy="Published public Creator information for editorial planning. This directory is read-only." />
    <form className="team-filter-bar team-filter-bar-compact ui44-panel" onSubmit={submit}>
    ...
    {loading ? <div className="ui44-loading-shell" role="status" aria-label="Loading Creators" /> : rows.length === 0 ? <EmptyMessage>No published Creators match this view.</EmptyMessage> : <div className="team-directory-grid">
      {rows.map(creator => <Link key={creator.profile_id} href={creator.profile_url} className="team-creator-card ui44-panel">
        ...
        <div><h2>{creator.display_name || creator.username || 'Unnamed Creator'}</h2><p>{creator.username ? `@${creator.username}` : 'No username'}</p>{creator.bio ? <small>{creator.bio}</small> : null}<strong>{creator.published_item_count} published Item{creator.published_item_count === 1 ? '' : 's'}</strong></div>
      </Link>)}
    </div>}
  </main></PageShell>;
}
