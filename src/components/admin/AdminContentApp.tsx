...
function ContentDirectory() {
  ...
  useEffect(() => {
    ...
    void listAdminContent({ query, status, type, page }).then(result => {
      if (!alive) return;
  ...
  function buildHref(nextPage: number, nextQuery = query, nextStatus = status, nextType = type) {
  ...
  return <PageShell><main className="admin-page">
    ...
    <form className="admin-filter-bar admin-filter-bar-wide ui44-panel" onSubmit={submit}>
    ...
    {loading ? <div className="ui44-loading-shell" role="status" aria-label="Loading content" /> : rows.length === 0 ? <EmptyMessage>No content matches this view.</EmptyMessage> : <>
      <div className="admin-record-list ui44-panel" role="list" aria-label="Content">
        <div className="admin-record-heading admin-content-grid" aria-hidden="true"><span>Content</span><span>Creator</span><span>Publication</span><span>Review</span><span>Created</span></div>
        {rows.map(item => <Link key={item.item_id} href={`/admin/content/${item.item_id}`} className="admin-record-row admin-content-grid" role="listitem">
          ...
          <span className="admin-row-count"><time dateTime={item.created_at}>{formatAdminDate(item.created_at)}</time><span aria-hidden="true">›</span></span>
        </Link>)}
      </div>
