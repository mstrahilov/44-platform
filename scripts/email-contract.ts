...
async function main() {
  ...
  assert.match(emailWebhook, /for \(const secret of resendWebhookSecrets\(\)\)[\s\S]*new Webhook\(secret\)\.verify\(rawBody/, 'Resend events require raw-body signature verification with bounded secret rotation');
