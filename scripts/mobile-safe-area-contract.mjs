...
const requirements = [
  ...
  ['desktop Account destination remains labelled', /aria-label=\{label\}[\s\S]*sidebar-account-avatar/, sidebar],
  ...
  ['mobile navigation has five equal columns', /grid-template-columns:\s*repeat\(5,\s*minmax\(0,\s*1fr\)\)/, css],
  ['mobile navigation order is Home Library Radio Community Account', /getOSApp\('store'\)[\s\S]*getOSApp\('library'\)[\s\S]*getOSApp\('radio'\)[\s\S]*getOSApp\('community'\)[\s\S]*getOSApp\('you'\)/, sidebar],
  ['mobile navigation has visible labels', /className="mobile-dock-label"/, sidebar],
  ['mobile Account routes own the Account navigation state', /pathname\.startsWith\('\/you'\)[\s\S]*return 'you'/, osApps],
  ...
  ['shared bottom shell includes navigation and safe area', /--os-mobile-shell-bottom:\s*calc\([\s\S]*var\(--os-mobile-dock-height\)\s*\+\s*env\(safe-area-inset-bottom\)/, css],
