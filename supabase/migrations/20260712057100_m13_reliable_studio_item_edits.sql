-- M13 release blocker: make existing Item edits server-authoritative.
-- This preserves the permanent Item row and child/audit history.

create table public.publishing_runtime_controls (
  singleton boolean primary key default true check(singleton),
  phase text not null default 'trusted_testing' check(phase in ('trusted_testing','review_required')),
  review_required boolean not null default false,
  updated_at timestamptz not null default now(),
  check((phase='review_required')=review_required)
);
insert into public.publishing_runtime_controls(singleton) values(true);
alter table public.publishing_runtime_controls enable row level security;
revoke all on public.publishing_runtime_controls from anon,authenticated;
grant all on public.publishing_runtime_controls to service_role;
comment on table public.publishing_runtime_controls is 'Explicit launch phase gate. Trusted approved creators publish directly until the audited review workflow is implemented and deliberately enabled.';

create or replace function public.update_owned_item(target_item_id uuid, patch jsonb)
returns void language plpgsql security definer set search_path=public as $$
declare
  allowed_keys constant text[] := array['title','item_category_id','item_type','price_cents','market_mode','local_price_cents','local_currency','available_locally_only','is_free','cover_url','experience_type','fulfillment_type','merch_fulfillment_mode','merch_shipping_scope','read_url','download_url','year','creator'];
  unexpected_key text;
begin
...
comment on function public.update_owned_item(uuid,jsonb) is 'Ownership-checked, field-whitelisted Studio edit path preserving permanent Item, publication state, and child identities.';
