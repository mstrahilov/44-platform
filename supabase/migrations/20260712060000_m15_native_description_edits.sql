-- M15 edit compatibility: allow the canonical Item description through the
-- ownership-checked trusted-testing RPC without widening any lifecycle fields.
create or replace function public.update_owned_item(target_item_id uuid, patch jsonb)
returns void language plpgsql security definer set search_path=public as $$
declare
  allowed_keys constant text[] := array['title','long_description','item_category_id','item_type','price_cents','market_mode','local_price_cents','local_currency','available_locally_only','is_free','cover_url','experience_type','fulfillment_type','merch_fulfillment_mode','merch_shipping_scope','read_url','download_url','year','creator'];
  unexpected_key text;
begin
...
comment on function public.update_owned_item(uuid,jsonb) is 'Ownership-checked, field-whitelisted Studio edit path including canonical native-content descriptions while preserving publication and child identity.';
