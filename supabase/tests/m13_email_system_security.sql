...
select set_config('request.jwt.claim.role','service_role',true);
select throws_ok(
  $$select public.queue_welcome_email('e4400000-0000-4000-8000-000000000001','https://44os.com/library')$$,
...
set local role service_role;
select set_config('request.jwt.claim.role','service_role',true);
select lives_ok($$select public.queue_welcome_email('e4400000-0000-4000-8000-000000000001','https://44os.com/library')$$,'confirmed account can queue welcome');
select lives_ok($$select public.queue_welcome_email('e4400000-0000-4000-8000-000000000001','https://44os.com/library')$$,'duplicate welcome intent is idempotent');
...
select lives_ok($$select public.queue_application_email('stale/retry','welcome',1,'e4400000-0000-4000-8000-000000000003','email-other@example.test','account','e4400000-0000-4000-8000-000000000003','{"displayName":"Other","libraryUrl":"https://44os.com/library"}')$$,'worker can queue a stale-claim recovery fixture');
...
select lives_ok($$select public.queue_application_email('stale/reconcile','welcome',1,'e4400000-0000-4000-8000-000000000003','email-other@example.test','account','e4400000-0000-4000-8000-000000000003','{"displayName":"Other","libraryUrl":"https://44os.com/library"}')$$,'worker can queue an expired idempotency-window fixture');
...
select lives_ok($$select public.queue_application_email('stale/manual-retry','welcome',1,'e4400000-0000-4000-8000-000000000003','email-other@example.test','account','e4400000-0000-4000-8000-000000000003','{"displayName":"Other","libraryUrl":"https://44os.com/library"}')$$,'worker can queue a manual-retry reconciliation fixture');
...
begin
  fixture_id:=public.queue_application_email('stale/final-ambiguous-failure','welcome',1,'e4400000-0000-4000-8000-000000000003','email-other@example.test','account','e4400000-0000-4000-8000-000000000003','{"displayName":"Other","libraryUrl":"https://44os.com/library"}');
...
select lives_ok($$select public.queue_application_email('provider/known-rejection','welcome',1,'e4400000-0000-4000-8000-000000000003','email-other@example.test','account','e4400000-0000-4000-8000-000000000003','{"displayName":"Other","libraryUrl":"https://44os.com/library"}')$$,'worker can queue a known provider-rejection fixture');
...
select lives_ok($$select public.queue_application_email('support-ack/suppressed','support_acknowledgement',1,'e4400000-0000-4000-8000-000000000001','email-member@example.test','support_case',null,'{"caseReference":"SUP-X","subject":"X","receivedAt":"now","supportUrl":"https://44os.com/support"}')$$,'suppressed address still records durable intent');
